...
const noRefs: ItemRefs = { approvedLinesElsewhere: 0, recipeIngredients: 0, countLines: 0, offers: 0 }
...
describe('planRollback — ITEM_CREATED', () => {
  ...
  it.each([
    ['an approved line on another session', { approvedLinesElsewhere: 1 }],
    ['a recipe ingredient', { recipeIngredients: 1 }],
    ['a count line', { countLines: 1 }],
    ['a remaining supplier offer', { offers: 1 }],
  ])('keeps a created item referenced by %s', (_label, over) => {
    const plan = planRollback(input({ records: [rec], current, refs: new Map([['new-item', { ...noRefs, ...over }]]) }))
    expect(plan.rows[0]).toMatchObject({ outcome: 'skipped', reason: 'referenced' })
    expect(plan.rows[0].write).toBeUndefined()
  })

  it('keeps a created item whose references were never checked', () => {
    ...
    const plan = planRollback(
      input({
        legacy: {
          ...
          priceAlerts: [],
          lines: [
            { action: 'UPDATE_PRICE', matchedItemId: 'i1', previousPrice: 15, matchedItem, itemName: 'Flour' },
            { action: 'ADD_SUPPLIER', matchedItemId: 'i2', previousPrice: 8, matchedItem },
            { action: 'UPDATE_PRICE', matchedItemId: 'i3', previousPrice: null, matchedItem },
            { action: 'CREATE_ITEM', matchedItemId: 'i4', previousPrice: 5, matchedItem },
            { action: 'UPDATE_PRICE', matchedItemId: null, previousPrice: 5, matchedItem },
          ],
        },
      })
    )
    ...
    const plan = planRollback(
      input({
        legacy: {
          ...
          priceAlerts: [],
          lines: [
            { action: 'UPDATE_PRICE', matchedItemId: 'i1', previousPrice: 15, matchedItem },
            { action: 'UPDATE_PRICE', matchedItemId: 'i1', previousPrice: 17, matchedItem },
          ],
        },
      })
    )
    ...
    const plan = planRollback(
      input({
        legacy: {
          status: 'REVIEW',
          priceAlerts: [],
          lines: [{ action: 'UPDATE_PRICE', matchedItemId: 'i1', previousPrice: 15, matchedItem }],
        },
      })
    )
    expect(plan.legacy).toBe(false)
    ...
    const plan = planRollback(
      input({
        ...
        legacy: {
          status: 'APPROVED',
          priceAlerts: [],
          lines: [{ action: 'UPDATE_PRICE', matchedItemId: 'i9', previousPrice: 15, matchedItem }],
        },
      })
    )
    expect(plan.legacy).toBe(false)
...
describe('executeRollback', () => {
  ...
  it('sends Prisma.JsonNull for a restored null Json column, and plain values for the rest', async () => {
    ...
    expect(calls[0].data?.packChain).toBe(Prisma.JsonNull)
    expect(calls[0].data?.pricing).toBe(Prisma.JsonNull)
