...
export function SuggestionRow({ item, locked, longLead = false, onOpen, onAdd, onRemove }: {
  ...
  return (
    ...
    <div
      className={`grid grid-cols-[minmax(0,1fr)_auto_28px] items-center gap-2 border rounded-[9px] py-2 pr-2 pl-2.5 border-l-[3px] ${item.isOnList || !enabled ? 'bg-bg border-line opacity-60' : 'bg-paper border-line'}`}
