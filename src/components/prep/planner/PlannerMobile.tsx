        ...
        <div className="pointer-events-auto">
          {locked ? (
            ...
            <div className="flex items-center gap-2 bg-paper border border-line rounded-xl px-3 py-2.5 shadow-lg">
              ...
              <button type="button" onClick={handlers.onRecall} disabled={locked} title={locked ? 'Pick a revenue center you can edit' : undefined} className={`inline-flex items-center gap-1.5 rounded-[9px] px-[11px] py-[7px] text-[12px] font-semibold border border-line-2 bg-paper text-ink-2 ${locked ? 'opacity-40' : ''}`}>
                <Undo2 size={13} /> Recall
              </button>
            </div>
          ) : (
            <button type="button" onClick={() => setDlg(true)} disabled={locked || !draft.length} title={locked ? 'Pick a revenue center you can edit' : undefined}
