    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-ink">Assign Allergens</h3>
          <button onClick={onClose} className="text-ink-4 hover:text-ink-3"><X size={16} /></button>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2 bg-ink text-white text-sm font-medium rounded-lg hover:bg-ink"
          >
            Apply to {count} item{count !== 1 ? 's' : ''}
          </button>
          <button onClick={onClose} className="px-4 py-2 border border-line rounded-lg text-sm text-ink-3 hover:bg-bg">
            Cancel
          </button>
        </div>
      </div>
