...
**Which lines qualify.** `InvoiceScanItem` rows where `approved = true`, `session.status = 'APPROVED'`, `matchedItemId ∈ itemIds`, `splitToSessionId IS NULL` (RC-split parents are excluded; their clones carry the scaled money and quantity, so the clones sum to the parent — the same filter `periodPurchases` in `cogs.ts` uses), `session.purchaseDate` within the window — purchases dated within the 30 days before now, the start day counted whole (`costWindow` subtracts `COST_WINDOW_DAYS` from `asOf` and floors to UTC midnight, matching the UTC-midnight calendar date `parseInvoiceDate` stores; the window is therefore up to 31 calendar days wide, and an invoice dated exactly 30 days ago is in whatever the time of day) — and **both** `rawLineTotal > 0` and `receivedQtyBase > 0`. A line contributes to both sums or to neither: credits and negative lines, unpriced lines (Veal Bones with no price must not drag the average toward $0), and lines never frozen are excluded and counted in `excluded`.
...
**Sizing (read-only run 2026-09-21, dump `docs/audits/2026-09-21-weighted-average-costing/wac-sizing-2026-09-22T03-07-56-784Z.json`).** 422 active non-PREP items: 135 on `AVG_30D`, 287 with no purchase in 30 days (`LAST`), 0 implausible, 0 excluded lines. Of the 135, all but 7 move by < 20 %. The recipes: 1 moves > 10 % (Cole Slaw Mix −62.7 %), 4 move 5–10 %, the rest < 5 %; every MENU dish moves < 7 % (The Coastal Benedict −6.1 % is the largest). The two big item movers are NOT price signal — they are wrong each-measures that made the frozen receipts wrong, which the average faithfully exposes: **Red Peppers** (each-measure 11 lb = the whole case, so a 1 × 11 lb case froze as 1 each → $32.95/each on 3 of 4 lines; avg +283 %) and **CABBAGE RED FDSVC** (each-measure 1 lb, so a 10 lb case froze as 10 heads instead of ~3 → $3.50/each; avg −69.5 %, which is what drives Cole Slaw Mix). Both are data fixes (correct the each-measure, re-freeze with `scripts/backfill-received-qty-base.ts --refreeze`), outside this branch. The other movers are real: Eggplant −43 % (North Arm Farms at $3.49–4.99/lb pools with Sysco's $70.30 case), Fingerling Potatoes +36 % (NAF $3.99/lb vs the Sysco $6.45/kg spine), Veal Bones −44 % (one priced line; the unpriced 501 lb line is excluded).

**Final review fixes.** The whole-branch review at `4c33d1b` returned "ready with fixes"; these landed on top of it:

1. **The N+1 on the list route.** A PREP-linked raw item never gets a `ctx.prices` entry (by design — its cost is its recipe's), so the missing-ids merge in `fetchRecipeWithCost` counted it missing *every time* and re-ran `windowedAvgCost` for it once per recipe that used it. `CostContext` now carries `asked: Set<string>` — every id put to `windowedAvgCost`, priced or not — and the merge skips ids already asked. `GET /api/recipes` additionally pre-seeds the context with the raw ingredient ids of every prep linked from the page (one extra `recipe.findMany`), so a nested recursion finds its prices already present instead of firing its own read.
2. **A cross-dimension nested yield no longer silently multiplies the cost.** `convertQty` passes a cross-dimension conversion through 1:1, so a prep whose `yieldUnit` had drifted out of its synced item's dimension (a stale sync — recipe yields `each`, item still in `g`) divided the batch cost by the bare yield number and came out off by the whole unit factor. `resolveLinkedRecipes` now requires `dimensionOf(nested.yieldUnit) === dimensionOf(spine.yieldUnit)` before using the recursion; otherwise it takes the spine, tagged `'LAST'`.
3. **`CostContext.asOf` is no longer dead.** No call site passed one, so each nested merge re-evaluated `new Date()` and a request could straddle midnight. `costContext` now pins `asOf = asOf ?? new Date()` and the field is non-optional; every merge in a request measures the same window.
4. **The window's wording.** `costWindow` floors `asOf − 30 d` to UTC midnight (matching the UTC-midnight `purchaseDate`), so it spans up to 31 calendar days. The maths is unchanged and pinned by a test; §1 above, the `costWindow` JSDoc and the CLAUDE.md paragraph now all say "purchases dated within the 30 days before now — the start day counted whole".
5. **CLAUDE.md's route list** named only the four recipe routes; `GET /api/inventory/[id]` also calls `windowedAvgCost` (to expose the item drawer's `costBasis`) and is now listed.
6. **The list route's accumulator is typed.** `const result = []` was an evolving `any[]`; it is now `RecipeRow[]` against an explicit interface, so a dropped or misspelled payload field fails to compile.
