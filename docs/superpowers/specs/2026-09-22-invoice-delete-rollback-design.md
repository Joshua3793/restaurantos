...
4. **Legacy path** (no records): today's `revertedPricing` over `previousPrice`, exactly as it stands today — `UPDATE_PRICE` lines **only**, never `ADD_SUPPLIER` — `outcome: 'best-effort'`; offers, rules and created items are `skipped, reason: 'approved before undo records existed'`. (An earlier draft of this design extended the legacy revert to `ADD_SUPPLIER` lines too; final review found that wrong — see "As built".)
...
- **The legacy "still equals the newPrice-implied value" guard is not implementable.** §2 step 4 described gating the best-effort revert on the item's current price still matching the line's `newPrice`-implied value (± 0.5%). `newPrice` is stored in the offer's own denomination (whatever unit that supplier's pack used), so comparing it to the item's current `pricePerBaseUnit` would mean redoing the exact per-line unit-conversion the OCR/matcher applied at approve time — and `PriceAlert.newPrice` only exists for moves ≥ 15%, so most lines have no independent witness at all. The legacy path is exactly today's `revertedPricing` rule, with no additional guard.
- **The legacy path reverts `UPDATE_PRICE` lines only — `ADD_SUPPLIER` is never reverted.** An earlier pass of this implementation extended the legacy revert to `ADD_SUPPLIER` lines too, on the theory that any repriced line should be covered by the best-effort path. Final whole-branch review found that wrong, for two independent reasons rooted in `invoice-matcher.ts`'s `buildMatchResult`: (1) `action` is set to `ADD_SUPPLIER` (rather than `UPDATE_PRICE`) precisely when the line did **not** move the item's price (`|priceDiffPct| ≤ 0.1%`, or `priceDiffPct === null` — no comparable prior price at all); approve's `shouldReprice` only writes the item's spine when the line's supplier is the item's primary offer (or the item has no offers yet), so an `ADD_SUPPLIER` line from a non-primary supplier never touched the spine in the first place, and reverting it anyway overwrites the item with a write that was never made. (2) The line's `previousPrice` is `offerLastPrice ?? Number(item.purchasePrice)` — **that supplier's own** last price, not necessarily the value the spine held before this session; reverting to it can plant the wrong supplier's number on the item. `Number(null)` is `0`, so a line with no prior price at all (`offerLastPrice` null and `item.purchasePrice` unset) could revert the item to a **zero** price — the legacy path (`legacyRows` in `src/lib/invoice/rollback.ts`, and the loader's `scanItems` filter in `rollback-load.ts`) now additionally requires `Number(previousPrice) > 0`, closing that off regardless of action. The net rule, and the only one that ships: `action === 'UPDATE_PRICE'` and `Number(previousPrice) > 0`, nothing else — exactly what `DELETE` did before this design, unchanged.
...
**Sizing (read-only run 2026-09-22, dump in `docs/audits/2026-09-22-invoice-delete-rollback/`):** 345 `APPROVED` sessions — all legacy (no undo records exist before the migration); 71 RC clones (delete now refuses them directly and removes them with their parent); 236 approved sessions carry at least one `ADD_SUPPLIER` line (today's revert silently skips those, and — per the correction above — the shipped legacy path deliberately continues to skip them, for the same reason today's revert does: an `ADD_SUPPLIER` line never re-priced the item's spine, so there is nothing on it to best-effort-revert); 0 offers point at a deleted session and 0 likely-orphaned clones, so no past delete has left visible damage to repair.

### Final whole-branch review fixes (post-merge, pre-deploy)

Four defects caught in a final whole-branch review, before this branch ships:

- **C1 — the legacy path's `ADD_SUPPLIER` inclusion, reverted** (see the correction under "Restore & the delete flow" above): `legacyRows` (`src/lib/invoice/rollback.ts`) and the loader's `scanItems` filter (`src/lib/invoice/rollback-load.ts`) now select `action === 'UPDATE_PRICE'` only, and additionally require `Number(previousPrice) > 0` (closing the `Number(null) === 0` hazard regardless of action).
- **I1 — `GET …/delete-plan` no longer ships spine data to every role.** The route gates on `requireSession()` alone (any signed-in role, including STAFF), but each `PlanRow.write.data` carried the actual restore payload — `lastPrice`, `pricing`, `purchasePrice`, `packChain` — cost fields STAFF cannot see anywhere else in the app. The GET response now strips `write` from every row (`rows: plan.rows.map(({ write, ...r }) => r)`); the preview only ever needed the outcome and its label. The `DELETE` responses were checked and do not leak `write` — a `skipped` `PlanRow` never carries one.
- **I2 — `maxDuration = 300`** added to both `src/app/api/invoices/sessions/[id]/route.ts` and `src/app/api/invoices/sessions/route.ts` (precedent: `src/app/api/inventory/[id]/merge/route.ts`), matching `TX_OPTIONS.timeout` (30s) — without it the Vercel function's shorter default can kill the request (and the transaction with it) before Prisma's own timeout ever has a chance to.
- **I3 — `saveMatchRule`'s two undo-capture reads (the sibling `findMany`, the target `findUnique`) are now caught.** Both were unguarded while the write itself is not — approve wraps the whole `saveMatchRule` call in `.catch(() => {})`, so an uncaught read failure here would silently lose a learned match approve would otherwise have written. Each read's failure is caught to a safe empty value (`[]` / `null`), and — because "read failed" and "read succeeded, found nothing" must never be confused — a failed `findUnique` no longer causes `created()` to fire after the upsert: `existingReadFailed` keeps the two cases apart, since treating an unknown prior state as "this rule is new" would let a later rollback DELETE a rule that predates this invoice.
