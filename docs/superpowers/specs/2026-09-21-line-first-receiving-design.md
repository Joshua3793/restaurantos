...
- **Applied to live data 2026-09-21:** `--refreeze --apply --skip-pack-path` re-froze **33 lines**, all via billed weight (backup `received-qty-refreeze-backup-2026-09-21T18-05-35-637Z.json`, kept outside the repo history). Beforehand, on the user's instruction, "Lettuce Burger" each-measure was corrected from 10 lb to **100 g** per each (5 lb frisée → 22.7 each, was 120). That correction surfaced one PACK-path line nobody had reviewed — "LETTUCE GREEN LEAF CROWNS", 1 cs, printed weight pack: 1 → 45.4 each — which the new abort guard caught; `--skip-pack-path` (added for exactly this) wrote only the reviewed weight moves and left that line as it was, for the user to decide. A follow-up dry run shows that single line as the only remaining difference.
